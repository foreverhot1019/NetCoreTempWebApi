...
namespace NetCoreTemp.WebApi.Models.Extensions
{
    public class Rar_FileHelper
    {
        ...
        public string AddFileToRar(List<String> ArrFiles, string RarNameTop = "Dec_Rec_rar_")
        {
            ...
            string PCNameStr = Environment.MachineName;
            //创建现有的文件
            Default_Folder = Path.Combine(Directory.GetCurrentDirectory(), "DownLoadRar\\" + DateTime.Now.ToString("yyyy-MM-dd"));
            if (!string.IsNullOrEmpty(PCNameStr))
                Default_Folder = Path.Combine(Default_Folder, PCNameStr);
            //加一个随机数文件夹
            Default_Folder = Path.Combine(Default_Folder, new Random().Next(1, 9999).ToString("0000"));
