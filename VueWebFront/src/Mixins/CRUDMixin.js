...
const ArrTagEditField = [] // 记录所有数组字段
// 枚举类型字段
const BaseArrField = {
  ...
  ArrField: [], // 所有字段
  get GetArrField() { return this.ArrField }, // 获取值 处理
  ...
  set SetArrField(value) { // 设置值 处理
    ...
    // Object.keys(CustomerFields).forEach((item, index) => {
      const OField = BaseArrField.ArrField.filter(val => {
...
var cRUDMixin = {
  ...
  data: function() {
    ...
    var pagiNation = { // 翻页控件数据
      pageSizes: [2, 10, 20, 50, 100, 200, 300, 500],
      pageSize: 10,
      currentPage: 1,
    ...
    var filters = { // 搜索数据
      ...
      pageNationToken: pagiNation.pageNationToken,
    ...
    tb.pagiNationTokens = {} // 记录翻页后的Token
    return tb
  }, // 数据集
  computed: { // 计算属性
    dgTitle: function() {
  ...
  watch: { // 监听属性变化
    ...
    'pagiNation.currentPage': {
      handler: function(newValue, oldValue) {
        ...
        this.filters.pageNationToken = this.pagiNationTokens[newValue] || ''
  ...
  methods: {
    ...
    tb_GetData: function(e) { // 获取数据
      ...
      // 筛选需要的字段
      const _paramData = (({ pageNationToken, limit }) => ({ pageNationToken, limit }))(filterData)
      var paramData = Object.assign({}, _paramData)
      var SearchFilter = thisVue.getSearchFilter()// 搜索{}转换为[]
      paramData.searhFilters = JSON.stringify(SearchFilter)
      BaseApi.Get(paramData).then(function(res) { // 成功
        try {
          ...
          const { arrData: rows, rowsCount: total, pageNationToken } = res
          // console.log(res)
          thisVue.tableData = rows
          if (objIsEmpty(_paramData.pageNationToken)) {
            thisVue.pagiNation.total = total
          }
          // 赋值pagiNationToken
          var key = filterData.page + 1
          thisVue.pagiNationTokens[key] = pageNationToken
        } catch (e) {
          thisVue.$message({
            duration: 0, // 不自动关闭
            showClose: true,
    ...
    getSearchFilter: function() { // 搜索条件{}转换为[]
       ...
       * 0:等于-Equal
       * 1:不等于-NotEqual
       * 2:小于等于-LessThanOrEqual
       * 3:小于-LessThan
       * 4:大于等于-GreaterThanOrEqual
       * 5:大于-GreaterThan
       * 6:不为空-IsNotNull
       * 7:为空-IsNull
       * 8:含有-Contains
      ...
      if (!(typeof (filterRules) === 'undefined' || filterRules === null || filterRules === JSON.parse('{}'))) {
        ...
        for (var i in ArrKey) {
          var key = ArrKey[i]
          const field = this.ArrSearchField.find(item => item.Name === key)
          if (!field) {
            continue
          }
          let _type = 0
          ...
          if (Array.isArray(value)) {
            ...
            if (_type === 3) { // datetime
              ...
              ArrFilter.push({ field: key, op: 12, value: val, valType: _type }) // Between
            } else {
              ArrFilter.push({ field: key, op: 11, value: val, valType: _type }) // In
            }
          } else {
            ArrFilter.push({ field: key, op: 0, value: value + '', valType: _type })
          }
        }
      }
      return ArrFilter
    ...
    handleViewClick: function(row, column, event) {
      // console.log('row',row)
    }, // 行查看按钮
    handledblclick: function(row, column, event) {
      ...
      const _currRowData = this.curr_rowdata_Original
      const ArrEnumField = this.ArrEnumField // 所有select/枚举
      const thisVue = this
      Object.keys(_currRowData).forEach(function(item, index) {
        const val = _currRowData[item] + ''
        if (!objIsEmpty(val)) {
      ...
      thisVue.ArrTagEditField.forEach(item => {
        const tagVal = _currRowData[item.Name]
    ...
    deleteRow: function(index, row) {
      ...
      const Id = keyField.Name || 'Id'
      // 提交数据
      BaseApi.Delete(row[Id]).then(res => {
        thisVue.tbLoading = false // 加载完毕
        const Success = res.isSuccess || res.Success
        if (Success) {
        ...
        thisVue.$message({
          duration: 0, // 不自动关闭
          showClose: true,
          message: `删除数据，出现错误${err.errMessage}`,
          type: 'error'
        })
      })
    }, // 行删除按钮-删除行搜索条件
    handledelSeltRow: function(e) {
      ...
      if (this.selctRows.length <= 0) {
        ...
        var thisVue = this
        thisVue.tbLoading = true // 加载中
        const ArrDelPromiseFunc = [] // 记录删除数据方法
        const keyField = thisVue.getkeyFiled()
        const Id = keyField.Name || 'Id'
        thisVue.selctRows.forEach(item => {
          if (!objIsEmpty(item[Id])) {
            ...
            if (!isNaN(item[Id]) && item[Id] > 0) {
              canDelete = true
            } else if (item[Id].indexOf('_') !== 0) {
              canDelete = true
            }
            if (canDelete) {
              const DelFunc = BaseApi.Delete(item[Id])
        ...
        Promise.all(ArrDelPromiseFunc).then(ArrRes => {
          thisVue.tbLoading = false // 加载完毕
          const errSome = ArrRes.filter(val => {
            const success = val.isSuccess || val.success
          ...
          const ErrMsg = Array.isArray(ArrErr) ? ArrErr.map(x => { return x.errMessage }).join(',') : ArrErr.errMessage
          thisVue.tbLoading = false // 加载完毕
          thisVue.$message({
            duration: 0, // 不自动关闭
            showClose: true,
    ...
    dlgSubmit: function(e) {
      ...
      if (batchSaveData) {
        ...
        const ArrPromiseFunc = [] // 记录异步方法
        batchSaveData.inserted.forEach((item, index) => {
          item[keyField.Name] += ''
          if (typeof item[keyField.Name] === 'string' && item[keyField.Name].substr(0, 1) === '_') {
            item[keyField.Name] = item[keyField.Name].substr(1)
          }
          const AddFunc = BaseApi.Add(item)
          ArrPromiseFunc.push(AddFunc)
        })
        batchSaveData.updated.forEach((item, index) => {
          const EditFunc = BaseApi.Edit(item[keyField.Name], item)
