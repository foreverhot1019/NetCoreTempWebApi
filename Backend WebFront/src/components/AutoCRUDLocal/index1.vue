...
export default {
  ...
  data: function() {
    var data = {
      ...
      ArrEnumField: [
        {
          Name: "IdCardType",
          ForeignKeyGetListUrl: "/Home/GetPagerEnum?EnumName=IdCardType",
        },
      ], // 所有Select
  ...
  methods: {
    el_FormFieldRules: function(rowConfig, isSearchForm) {
      ...
      if (
        t_isSearchForm === "undefined" ||
        isSearchForm == null ||
        t_isSearchForm !== "boolean"
      )
        isSearchForm = false
      var ArrRules = []
      if (!rowConfig.Editable) return ArrRules
      if (rowConfig.Required && !isSearchForm) {
        ArrRules.push({
          required: true,
          message: "请输入" + rowConfig.DisplayName || rowConfig.Name,
          trigger: ["blur", "change"]
        })
      }
      ...
      if (name.indexOf("password") === 0)
        ArrRules.push({
          validator: this.$Validtors.PasswordValidator,
          trigger: ["blur", "change"],
        })
      if ((name.indexOf("idcard") === 0) && (rowConfig.inputType === "text"))
        ArrRules.push({
          validator: this.$Validtors.IdCardValidator,
          trigger: "blur"
        })
        ...
        if (rowConfig.MaxLength) {
          ...
          if (rowConfig.MinLength)
            rule.message =
              ...
              rowConfig.MaxLength +
              " 之间"
          else rule.message = "字符长度 必须小于 " + rowConfig.MaxLength
        }
        ArrRules.push(rule)
      }
      return ArrRules
      ...
      Object.keys(curr_rowdata).forEach(function(item, index) {
        ...
        if (!ObjectIsEmpty(val)) {
          if (val.indexOf("/Date(") >= 0) {
            ...
            if (d.isValid()) curr_rowdata[item] = d.toDate();
          }
          var ArrFilter = thisVue.ArrEnumField.filter(function(field) {
    ...
    handledelSeltRow: function(e) {
      ...
      if (this.selctRows.length <= 0) {
        ...
        var deltRowIndex = this.tb_OrdCustomer_data.map(function(item, i) {
          var has_el = thisVue.selctRows.some(function(el, x) {
            return el.Id === item.Id;
          });
          if (has_el) return i;
          else return null;
        });
        deltRowIndex = deltRowIndex.filter(function(item) {
          return item != null;
        });
    ...
    IdCardTypeChange: function(value) {
      if (!ObjectIsEmpty(value)) {
        ...
        if (
          !(
            ObjectIsEmpty(ArrOptionData) ||
            JSON.stringify(ArrOptionData) == "{}"
          )
        ) {
          var ArrOption = ArrOptionData.ArrOption;
          if (!ObjectIsEmpty(ArrOption)) {
            var QOption = ArrOption.filter(function(item) {
        ...
        if (valid) {
          ...
          thisVue
            ...
            .catch(function() {
              ...
              else thisVue.DialogVisible = false;
    ...
    OrdCustomerQSearch: function(queryString, cb) {
      ...
      if (ObjectIsEmpty(queryString)) cb([]);
      else {
    ...
    OrdCuntomerSelt: function(data) {
      ...
      if (ObjectIsEmpty(OrderCuntomer.Birthday))
        this.$set(
          OrderCuntomer,
          "Birthday",
          this.$formatter.dateformatter(data, null, data.Birthday, 0)
        ); // 出生年月
      ...
      if (ObjectIsEmpty(OrderCuntomer.Limit_S))
        this.$set(
          OrderCuntomer,
          "Limit_S",
          this.$formatter.dateformatter(data, null, data.Limit_S, 0)
        ); // 签发日期
      if (ObjectIsEmpty(OrderCuntomer.Limit_E))
        this.$set(
          OrderCuntomer,
          "Limit_E",
          this.$formatter.dateformatter(data, null, data.Limit_E, 0)
        ); // 有效期
    ...
    OrdCuntomerChange: function() {
      if (!this.lockNameEng || ObjectIsEmpty(this.OrderCuntomer.NameEng)) {
        // 锁定部根据NameChs转拼音
